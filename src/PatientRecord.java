...
public class PatientRecord implements Comparable<PatientRecord> {
    // data fields
    private static int patientCounter = 1; // counts the number of patients created, Begins at 1,
    // and
    ...
    /**
     * Creates a new patient record and assigns it a CASE_NUMBER, as the counter will advance when
     * the static helper method generateCaseNumber() is called.
     *
     ...
     * @return {@code true} if other is instanceof PatientRecord and this PatientRecord and other
     * have the exact same CASE_Number
     */
    @Override
    public boolean equals(Object other) {
        return (other instanceof PatientRecord) && this.CASE_NUMBER
                == ((PatientRecord) other).CASE_NUMBER;
    }


    /**
     * Compares this PatientRecord to another patientRecord provided as input. PatientRecords are
     * first compared with respect to their triage levels. The smaller PatientRecord is the one
     * having a smaller triage. TriageLevels can be compared using the TriageLevel.compareTo()
     * method. By default, RED is less than YELLOW, and YELLOW is less than green.
     * <p>
     ...
     * arrival; a negative integer if this PatientRecord is less than other; and a positive integer
     * if this PatientRecord is greater than other.
     */
    @Override
    public int compareTo(PatientRecord other) {
        //If this record triage is smaller than the other return negative
        if (this.triage.compareTo(other.triage) < 0) {
            return -1;
            //If this record triage is larger than the other return positive
        } else if (this.triage.compareTo(other.triage) > 0) {
            return 1;
            //Else has same triage level
        } else {
            //If lower order of arrival return negative
            if (this.orderOfArrival < other.orderOfArrival) {
                return -1;
                //If higher order of arrival return positive
            } else if (this.orderOfArrival > other.orderOfArrival) {
                return 1;
                //Else they're the same and return 0
            } else return 0;
        }
    }
