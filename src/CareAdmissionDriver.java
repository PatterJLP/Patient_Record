        ...
        if (commands.length < 4) {
            ...
            try {
                ...
                if (commands[2].length() != 1) {
                    System.out.println(SYNTAX_ERROR_MSG + " Gender can be M (for Male), F (for " +
                            "Female), or X (for Other), only.");
                } else {
                    ...
                    PatientRecord add = new PatientRecord(gender, age, triage);
                    queue.addPatient(add);
                }
            } catch (NumberFormatException e) {
                System.out.println(SYNTAX_ERROR_MSG + " Invalid age!");

            } catch (IllegalArgumentException e) {
                System.out.println(SYNTAX_ERROR_MSG + " Invalid triage level! Should be either " +
                        "RED/YELLOW/GREEN");
            }
        }
            ...
            try {
                switch (command.charAt(0)) {
                    ...
                    case '2': // [2] Show the next patient record
                        ...
                        // the priority
                        // queue
                        PatientRecord next = queue.peek();
                        System.out.println(next.toString());
                        System.out.println(/* */);
                        break;
                    case '3': // [3] See next Patient
                        // remove the best record from the priority queue
                        PatientRecord seen = queue.removeBestRecord();
                        PatientRecord nextPatient = seen;// update this
                    ...
                    case '6': // [6] Clear the Care Admission Queue
                        System.out.println("Sorry! " +
                                "We are closed due to out of control circumstances!");
                        queue.clear();
                        break;
                    default:
